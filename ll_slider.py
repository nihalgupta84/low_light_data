...
def create_gui_app(frame1):
    ...
    def update_image_with_delay():
        """
        Update the image with a delay to avoid excessive updates.
        """
        global update_timer
        if update_timer is not None:
            update_timer.cancel()
        update_timer = threading.Timer(0.5, update_image)
        update_timer.start()
...
def save_image_and_settings(noise_level, gaussian_std, illumination_factor, blur_kernel_size, contrast_factor, color_factor, red_gain, green_gain, blue_gain):
    """
    Save the processed image and its corresponding settings.
    """
...
image_file_path = './saved_images/casc/traffic.jpg'
...
Noise Level: Increasing this slider will increase the amount of shot noise applied to the image, making it appear grainy and noisy.

Gaussian STD: Increasing this slider will increase the standard deviation of the Gaussian noise applied to the image, adding random noise to the image.

Illumination Factor: Decreasing this slider will make the image darker, simulating low-light conditions. Increasing it will make the image brighter.

Blur Kernel Size: Increasing this slider will increase the amount of motion blur applied to the image, simulating camera shake or motion.

Contrast Factor: Increasing this slider will increase the contrast of the image, making the dark areas darker and the bright areas brighter.

Color Factor: Decreasing this slider will decrease the color saturation of the image, making the colors appear more muted or washed out.

Red Gain: Adjusting this slider will change the intensity of the red channel in the image, affecting the overall color balance.

Green Gain: Adjusting this slider will change the intensity of the green channel in the image, affecting the overall color balance.

Blue Gain: Adjusting this slider will change the intensity of the blue channel in the image, affecting the overall color balance.

By combining these adjustments, you can effectively simulate various low-light or dark scenarios using the provided sliders.
"""